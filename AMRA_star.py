...
class AMRA_Star:
    def __init__(self, map, color_map, resolutions, heuristics, heuristic_to_res, start, goal, size_of_UAV, w1=500, w2=500):
        ...
        # self.out = cv2.VideoWriter('output_video.avi', cv2.VideoWriter_fourcc(*'DIVX'), 30, self.framesize)
    ...
    def plot_path(self, path):
        ...
        Annotate(plot_map, self.start, self.goal, (2*self.buffer[0]+1, 2*self.buffer[0]+1), self.w1, self.w2)
        plt.figure()
        plt.imshow(plot_map)
        plt.show()
    ...
    def Expand(self, x, i):
        ...
        self.video_map[x[0], x[1]] = [col, 0, 0]
        # if self.num_expansions % 100 == 0:
        ...
        for x_prime in self.Succs(x, r):
            if self.g[x_prime[0], x_prime[1]] > self.g[x[0], x[1]] + self.cost(x, x_prime):
                ...
                self.video_map[x_prime[0], x_prime[1]] = [0, 0, col]
    ...
    def Main(self):
        ...
        try:
            while self.w1 > 1 and self.w2 > 1:
                ...
                if self.ImprovePath():
                    ...
                    for point in path:
                        ...
                        self.video_map[point[0], point[1]] = [255, 0, 255]
                    # for i in range(60):
                    #     self.out.write(self.video_map)
            ...
            # self.out.release()
        print(self.num_expansions)
        print(self.g[goal[0], goal[1]])
        # self.out.release()
        print('Video Done')


def Annotate(map, start, goal, uav_size,  w1_val, w2_val):
    ...
    cv2.putText(map, f'UAV_size= ({uav_size[0]},{uav_size[1]})', (200, 960), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
def euclidean_2(node, goal):
    return 2 * np.linalg.norm(np.array(goal) - np.array(node))  # euclidean distance
...
# resolutions = [(1, 1),(1, 1)]
heuristics = [euclidean, euclidean_2, manhattan, manhattan]
